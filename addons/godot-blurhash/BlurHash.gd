...
## Encodes the supplied texture to a BlurHash. components_x and components_y
## control how many colors to extract and add on each axis for the blurred image. 
func encode(tex: Texture, components_x: int = 4, components_y: int = 3) -> String:
	if components_x < 1 or components_x > 9 or components_y < 1 or components_y > 9:
		return ""
	
	var tex_data := tex.get_data().get_data()
	var format := tex.get_data().get_format()
...
## Decodes the supplied BlurHash. width, height and flags are used when
## creating the texture. It's recommended to use low width/height values since
## the image doesn't have much detail anyways, and then scale the final image to
## the necessary size. That way, such operation is done on the GPU which is much,
## much faster than on CPU. The punch parameter controls the output's contrast.
## Increase it to make the colors stand out more.
func decode(blurhash: String, width: int, height: int, flags: int = 7, punch: int = 1) -> Texture:
	...
	var img := Image.new()
	img.create_from_data(width, height, false, Image.FORMAT_RGB8, data)
...
func _multiply_basis_func(component_x: int, component_y: int, width: int, height: int, data: PoolByteArray, format: int) -> Vector3:
	var r := 0.0
	var g := 0.0
	var b := 0.0
